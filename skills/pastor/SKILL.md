...
- Do not run, send to, attach to, retry, close or prune tasks, tick (not even `--dry-run`), run or reload jobs, install, link, uninstall or unlink plugins, or edit machines, flocks or jobs. pastor refuses these from your pane with `agent_refused` unless the user set `agents_change_fleet = true`; do not work around it. Reading (`task list`, `show`, `read`) is fine.
