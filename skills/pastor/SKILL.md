...
The head picks up job file edits by itself. A file that stops parsing keeps its last good version and shows the error in `pastor job list`. Changes to `flock.toml` and `pastor.toml` need a head restart in this version.
