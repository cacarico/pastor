You are running unattended as task 2 of the plan nested-herdr. Nobody will answer questions.
...
Read docs/superpowers/plans/2026-09-26-nested-herdr.md: the Global Constraints and Task 2 only. Do Task 2's steps in order, test first. Run `make check` and keep it green; never commit while it fails. Commit as the steps say, with conventional commit messages whose body says why.

When the task is done: tick Task 2's boxes in the plan and commit that. Then record it in the ledger and push, in this order:

1. `git fetch origin` and `git rebase origin/pastor/nested-herdr`, so the ledger you append to is the latest one.
2. Append the line `Task 2: complete (<first commit>..<last commit>, make check: pass)` to docs/superpowers/plans/2026-09-26-nested-herdr.ledger.md and commit it.
...
If something is missing or a step cannot be done as written, do not guess past it: record `Task 2: blocked: <why>` in the ledger the same way (fetch and rebase, append and commit, push), and stop.
