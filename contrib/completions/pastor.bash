_pastor() {
    ...
    case "${cmd}" in
        pastor)
            opts="-h -V --help --version serve run list task machine attach open tick reload job completions help"
        ...
        pastor__subcmd__help)
            opts="serve run list task machine attach open tick reload job completions help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
