...
complete -c pastor -n "__fish_pastor_needs_command" -f -a "list" -d 'List live tasks across the flock; --all adds finished ones'
...
complete -c pastor -n "__fish_pastor_using_subcommand list" -l all -d 'Every task, finished ones too (done, failed, stale, closed)'
...
complete -c pastor -n "__fish_pastor_using_subcommand help; and not __fish_seen_subcommand_from serve run list task machine attach open tick reload job completions events setup help" -f -a "list" -d 'List live tasks across the flock; --all adds finished ones'
