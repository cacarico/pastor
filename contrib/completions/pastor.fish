...
complete -c pastor -n "__fish_pastor_using_subcommand help; and not __fish_seen_subcommand_from serve task machine flock open tick job completions events setup connector trust help" -f -a "setup" -d 'Install pastor or herdr as a user service (systemd, or launchd on macOS)'
