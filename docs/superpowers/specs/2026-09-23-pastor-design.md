...
- First run passes `since = now - backfill` and `cursor = null`.
...
- `pastor run "<prompt>" [--repo P] [--machine M] [--agent K] [--worktree]`
  creates a one-off task with no job file.
...
pastor run "<prompt>" [...]         one-off task
```
...
```
pastor list [--job NAME] [--machine M] [--blocked|--done|--all]   hides closed by default
...
pastor attach t-123                         ssh -t <machine> herdr --session <s> agent attach t-123
...
`ssh pi-1 pastor list` and `ssh -t pi-1 pastor attach t-123` (the head then
hops to the task's machine). `pastor open` reads `flock.toml` locally, so on
the laptop it needs a copy of that file or you use `herdr --remote` directly.
A `--host` flag that does the ssh-exec for you is listed under Later.
...
| event | when |
|---|---|
| task.queued, task.started, task.running | dispatch progress |
...
`pastor list --blocked` shows task, machine, time blocked, and the last lines
...
version kept. `pastor reload` forces it. Plugins change only through
...
```
pastor setup systemd            head: pastor.service (pastor serve), enable + start
pastor setup systemd --herdr    any machine: herdr.service (herdr server), enable + start
```
...
pastor reload
pastor run "<prompt>" [--repo P] [--machine M] [--agent K] [--worktree]
pastor list [--job NAME] [--machine M] [--blocked|--done|--all]
pastor attach <task>
pastor open <machine>
